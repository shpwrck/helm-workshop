...
=== Option: Bash (Linux, macOS, or Git Bash)

==== If you are using bash only (no PowerShell)

The bash commands paired with the PowerShell steps in *Option: PowerShell* (above) are: append `source <(helm completion bash)` via `~/.bashrc` (see the first bash code block) and `source ~/.bashrc` (or run `source <(helm completion bash)` once in the current shell) as in the second bash code block. You can also add those lines to `~/.bash_profile` on macOS if you use a login shell.
